...
export class CybergridState extends GameState {
    ...
    public constructor(scene: Scene) {
        ...
        const camera = new UniversalCamera("sceneCamera", new Vector3(0, 5, -20), scene);
        camera.fov = 0.6;

        const ambient_light = new HemisphericLight("ambientLight", new Vector3(0, 0, -1), scene);
        const point_light = new PointLight("pointLight", new Vector3(10, 40, -20), scene);
        ...
        const angular_velocity = pyramid_entity.add_component(AngularVelocityComponent);
    ...
    private generate_grid_lines(left_x: number, bottom_y: number, right_x: number, top_y: number, spacing: number) {
        for (let y = bottom_y, end = top_y, step = spacing, asc = step > 0; asc ? y <= end : y >= end; y += step) {
            const line_geometry = MeshBuilder.CreateLines(
                "line " + left_x + " " + bottom_y + " " + right_x + " " + top_y,
                {
                    points: [
                        new Vector3(left_x, 0, y),
                        new Vector3(right_x, 0, y),
                    ],
                    colors: [
                        new Color4(0, 0, 0.8, 0.8),
                        new Color4(0, 0, 0.8, 0.8),
                    ],
                },
            );
        ...
        for (let x = left_x, end1 = right_x, step1 = spacing, asc1 = step1 > 0; asc1 ? x <= end1 : x >= end1; x += step1) {
            const line_geometry = MeshBuilder.CreateLines(
                "line " + left_x + " " + bottom_y + " " + right_x + " " + top_y,
                {
                    points: [
                        new Vector3(x, 0, bottom_y),
                        new Vector3(x, 0, top_y),
                    ],
                    colors: [
                        new Color4(0, 0, 0.8, 0.8),
                        new Color4(0, 0, 0.8, 0.8),
        ...
        for (let y = bottom_y, end2 = top_y, step2 = spacing, asc2 = step2 > 0; asc2 ? y <= end2 : y >= end2; y += step2) {
            const line_geometry = MeshBuilder.CreateLines(
                "line " + left_x + " " + bottom_y + " " + right_x + " " + top_y,
                {
                    points: [
                        new Vector3(left_x, 10, y),
                        new Vector3(right_x, 10, y),
                    ],
                    colors: [
                        new Color4(0, 0, 0.8, 0.8),
                        new Color4(0, 0, 0.8, 0.8),
        ...
        for (let x = left_x, end3 = right_x, step3 = spacing, asc3 = step3 > 0; asc3 ? x <= end3 : x >= end3; x += step3) {
            const line_geometry = MeshBuilder.CreateLines(
                "line " + left_x + " " + bottom_y + " " + right_x + " " + top_y,
                {
                    points: [
                        new Vector3(x, 10, bottom_y),
                        new Vector3(x, 10, top_y),
                    ],
                    colors: [
                        new Color4(0, 0, 0.8, 0.8),
                        new Color4(0, 0, 0.8, 0.8),
