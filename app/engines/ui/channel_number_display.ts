...
export class ChannelNumberDisplayEngine extends Engine {
    public update(dt: number) {
        for (const channel_number_component of this.read_components_mutable(ChannelNumberComponent).iterable()) {
            ...
            for (const show_channel_number_message of this.read_messages(ShowChannelNumberMessage).iterable()) {
                channel_number_component.text_block.isVisible = true;
                channel_number_component.time = 5;
            }

            for (const change_channel_number_message of this.read_messages(ChangeChannelNumberMessage).iterable()) {
