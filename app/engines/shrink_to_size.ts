...
export class ShrinkToSizeEngine extends Engine {
    public update(dt: number) {
        for (const shrink_to_size_component of this.read_components(ShrinkToSizeComponent).iterable()) {
            const mesh_component = this.get_entity(shrink_to_size_component.entity_id)!.get_component(MeshComponent);
            if (mesh_component.mesh && mesh_component.mesh.scaling.x > shrink_to_size_component.size) {
                const shrink_message = this.emit_component_message(GrowObjectMessage, mesh_component);
