...
interface IIcecastSongData { artistName: string; songName: string; }
interface IIcecastResponse { icestats: {source: {title: string; }; }; }
...
export class IcecastHelper {
    public static getSongData(successCallback: (data: IIcecastSongData) => any) {
        ...
        xhr.onload = () => {
            if (xhr.status === 200) {
                const songData = (JSON.parse(xhr.response) as IIcecastResponse).icestats.source.title;
